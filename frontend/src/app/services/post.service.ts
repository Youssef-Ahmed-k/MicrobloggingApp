...
export class PostService {
  ...
  tweet(payload: PostRequest) {
    return this.http.post('http://localhost:2020/posts/create', payload);
  }

  getRepliesForTweet(tweetId: number): Observable<Array<PostResponse>> {
    return this.http.get<Array<PostResponse>>(
      'http://localhost:2020/posts/replies-for-tweet/' + tweetId
    );
  }

  getTweetsByUsername(username: string): Observable<Array<PostResponse>> {
    return this.http.get<Array<PostResponse>>(
      'http://localhost:2020/posts/tweets-by-username/' + username
    );
  }

  getRetweetsByUsername(username: string): Observable<Array<PostResponse>> {
    return this.http.get<Array<PostResponse>>(
      'http://localhost:2020/posts/retweets-by-username/' + username
    );
  }

  getRepliesByUsername(username: string): Observable<Array<PostResponse>> {
    return this.http.get<Array<PostResponse>>(
      'http://localhost:2020/posts/replies-by-username/' + username
    );
  }

  getLikesByUsername(username: string): Observable<Array<PostResponse>> {
    return this.http.get<Array<PostResponse>>(
      'http://localhost:2020/posts/liked-by-username/' + username
    );
  }

  getAll(): Observable<PostResponse[]> {
    return this.http.get<PostResponse[]>('http://localhost:2020/posts');
  }
}
