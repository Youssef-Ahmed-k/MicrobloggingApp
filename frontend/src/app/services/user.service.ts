...
export class UserService {
  ...
  getUser(username: string): Observable<User> {
    return this.http.get<User>('http://localhost:2020/user/' + username);
  }

  getBannerPicture(username: string) {
    return this.http.get(
      'http://localhost:2020/user/banner-picture/get/' + username,
      { responseType: 'blob' }
    );
  }
  ...
  getProfilePicture(username: string) {
    return this.http.get(
      'http://localhost:2020/user/profile-picture/get/' + username,
      { responseType: 'blob' }
    );
  }
  ...
  saveBannerPicture(bannerPicture: File) {
    ...
    return this.http.post(
      'http://localhost:2020/user/banner-picture/save',
      this.payload
    );
  }
  ...
  saveProfilePicture(profilePicture: File) {
    ...
    return this.http.post(
      'http://localhost:2020/user/profile-picture/save',
      this.payload
    );
  }
  ...
  editProfile(payload: any) {
    console.log(payload);
    return this.http.post('http://localhost:2020/user/edit-profile', payload);
  }
}
