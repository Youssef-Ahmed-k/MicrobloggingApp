...
import { ChangePasswordDto } from '../payloads/request/changePassword';
...
export class AuthService {
  ...
  signUp(payload: SignUpPayload) {
    return this.http.post('http://localhost:2020/auth/sign-up', payload);
  }
  ...
  signIn(payload: SignInRequestPayload) {
    ...
    return this.http
      .post<SignInResponsePayload>(
        'http://localhost:2020/auth/sign-in',
        payload
      )
      .pipe<SignInResponsePayload>(
        map((response) => {
  ...
  logout() {
    const self = this;
    return this.http
      .post('http://localhost:2020/auth/logout', {
  ...
  refreshAccessToken(): Observable<SignInResponsePayload> {
    return this.http
      .post<SignInResponsePayload>('http://localhost:2020/auth/refresh-token', {
        refreshToken: this.localStorage.retrieve('refreshToken'),
        username: this.getUsername(),
      })
      .pipe(
  ...
  findAllUsernames(): Observable<String[]> {
    return this.http.get<String[]>('http://localhost:2020/auth/usernames');
  }

  changePassword(dto: ChangePasswordDto) {
    return this.http.patch('http://localhost:2020/auth/changePassword', dto);
  }
}
